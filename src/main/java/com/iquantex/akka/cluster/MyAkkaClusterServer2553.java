 ...
 * @author quail
 */
public class MyAkkaClusterServer2553 extends UntypedActor {
    ...
    void register(Member member){
        if(member.hasRole("client")){
            getContext().actorSelection(member.address() + "/user/myAkkaClusterClient").tell(TransformationMessage.BACKEND_REGISTRATION,getSelf());
        }
    }

    public static void main(String[] args) {
        System.out.println("Start MyAkkaClusterServer");
        ActorSystem system = ActorSystem.create("akkaClusterTest", ConfigFactory.load("reference-2553.conf"));
        system.actorOf(Props.create(MyAkkaClusterServer2553.class), "myAkkaClusterServer");
